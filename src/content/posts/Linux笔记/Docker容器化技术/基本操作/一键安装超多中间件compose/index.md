---
title: "Linux笔记 - Docker容器化技术 - 基本操作 - 一键安装超多中间件compose"
category: "Linux笔记"
date: 2026-04-14
...
| ------------------------------ | ---------- | -------------------- | ------------- | -------------- |
| Redis(redis)                   | k-v 库      | ip:6379              | 单密码模式：123456  | 已开启AOF         |
| MySQL(mysql)                   | 数据库        | ip:3306              | root/123456   | 默认utf8mb4字符集   |
| Rabbit(rabbit)                 | 消息队列       | ip:15672             | rabbit/rabbit | 暴露5672和15672端口 |
| OpenSearch(opensearch-nore1/2) | 检索引擎       | ip:9200              |               | 内存512mb；两个节点   |
| opensearch-dashboards          | search可视化  | ip:5601              |               |                |
| Zookeeper(zookeeper)           | 分布式协调      | ip:2181              |               | 允许匿名登陆         |
| kafka(kafka)                   | 消息队列       | ip:9092<br>外部访问：9094 |               | 占用内存512mb      |
| kafka-ui(kafka-ui)             | kafka可视化   | ip:8080              |               |                |
| nacos(nacos)                   | 注册/配置中心    | ip:8848              | nacos/nacos   | 持久化数据到MySQL    |
| nacos-mysql(nacos-mysql)       | nacos配套数据库 | ip:13306             | root/root     |                |
| prometheus(prometheus)         | 时序数据库      | ip:9090              |               |                |
| grafana(grafana)               |            | ip:3000              | admin/admin   |                |