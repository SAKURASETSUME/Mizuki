---
title: "Linux笔记 - 常用web服务应用搭建 - LVS-keepalived-HAproxy - HAproxy - 实操 - HAproxy+keepalived 实现动静分离"
category: "Linux笔记"
date: 2026-04-13
...
- HTML A 192.168.122.10/24
- HTML B 192.168.122.20/24
- PHP A 192.168.122.30/24
- PHP B 192.168.122.40/24
...
#PHP服务器
yum install -y php
echo print("1"); > /var/www/html/index.php
...
    stats uri /admin?stats #设置统计页面的uri为/admin?stats 这里配置的是HAproxy的web端uri 访问的时候直接用ip/uri就可以了
...
 default_backend html-server #默认的后端服务器是html-server 如果用户访问的不是以html结尾的url时 那么就把请求转发给html服务器
  ...
  backend php-server
		  ...
		  server php-B 192.168.122.40:80 weight 1 cookie 1 check inter 2000 rise 2 fall 5
...
        server html-B web2:80 weight 1 cookie 4 check inter 2000 rise 2 fall 5  # 服务器名在这里配置之后 需要单独做域名解析 如果不直接写域名 那就直接填ip也行
```